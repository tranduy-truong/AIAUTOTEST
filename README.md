...
- Oracle Resolver không tin trực tiếp expected do AI đề xuất. Expected chỉ được biên dịch khi có đoạn requirements của tester làm bằng chứng hoặc static evaluator chứng minh được từ một tập AST thuần, an toàn. Network/filesystem/process không bị chạy ngầm; quan sát sandbox chỉ được gắn nhãn characterization. Bằng chứng từng case nằm trong `oracle-resolution.json`; các điểm cần tester xác nhận được tách riêng trong `oracle-requests.json`.
